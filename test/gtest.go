...
func (tu TestUtil) MockClient(t *testing.T) *gchttp.ClockifyHTTPClient {
	load()
	l := &TestLogger{Testing: t}
	httpClient := &http.Client{Transport: newMockTransport(getResponse())}
	optionsWithClient := []gchttp.ClientOptionFunc{gchttp.SetHTTPClient(httpClient), gchttp.SetTraceLogger(l)}
	client, err := gchttp.NewClient("abc1234567890def", optionsWithClient...)
	if err != nil {
		panic(err)
	}
	return client
